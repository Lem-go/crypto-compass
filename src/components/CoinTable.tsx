              ...
              : coins?.map((coin) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-sm text-muted-foreground font-mono hidden sm:table-cell">
                        {mcapStr}
                      </td>
...
const SparkLine = ({ prices, positive }: { prices: number[]; positive: boolean }) => {
  ...
  const normalize = (value: number) => 2 + ((value - min) / range) * 26;
  const step = 80 / (prices.length - 1);
  const points = prices
    .map((price, i) => `${i * step},${30 - normalize(price)}`)
    .join(" ");
